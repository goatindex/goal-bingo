...
test('Test Scene Transitions', async ({ page }) => {
  ...
  // Test scene transition by calling scene.start() from the game object
  const transitionResult = await page.evaluate(() => {
    if (window.game) {
      try {
        // Try to start the BingoGridScene
  ...
  // Take a screenshot
  await page.screenshot({ path: 'tests/outputs/scene-transitions-test.png' });
