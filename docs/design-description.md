...
| ~~Q10~~ | ~~Power-up prices — constrained by the recovery floor, not free to tune~~ — **resolved**: grid expansion 250 board balance (`D-2026-09-21-9`), swap 10 board balance (`D-2026-09-21-10`), recycle 5 board balance ported from `sim/jam_sim.py`'s own default (`D-2026-09-21-8`) — none move the recovery floor, which the A4/A5 sensitivity runs already showed holds independent of these prices | §6.2, §10.3 |
...
| ~~Q17~~ | ~~How far the free recycle allowance can be upgraded, what each step costs, and whether it is capped~~ — **resolved** by `D-2026-09-21-11`: caps at 3 free recycles per 24h (two purchasable steps above the default of 1), each step 100 board balance | §6.2 |
